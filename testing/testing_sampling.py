...
# Now i want to try with 2 unknown community card + 2 unknown opponent pocket cards
# Theoretically, 47 possible cards left in deck
# 47*46/2 possibilities for the turn and river and and still 45*44/2 for opponent, 
# so total should be 47*46*45*44/4 = 1070190 (rough estimtate is takes 30 seconds)

deck = [Card(i,j, True) for j in ["Club", "Diamond", "Heart", "Spade"] 
                     for i in range(2, 15)] #create a standard deck

shuffle(deck)
community = [deck.pop(), deck.pop(), deck.pop()] #2 unknown community card
hand1 = [deck.pop(), deck.pop()]
print("hand1 =", hand1, file=fp)
print("community pile =", community, file=fp)
print("Finding hand strength of hand1 (probability it wins)\n", file = fp)
...
for cards in pocketcombinations(deck): #find all combos for turn+river
    temp = deck.copy()
    temp.remove(cards[0])
    temp.remove(cards[1])
    rest = list(cards)
    for hand2 in pocketcombinations(temp): #for each of those combos, find all combos for opponent pocket cards
        result = getwinner(hand1+community+rest, list(hand2)+community+rest)
        if result != "Loss":
            wins +=1
        total+=1
P = wins/total
